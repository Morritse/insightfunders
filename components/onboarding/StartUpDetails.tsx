...
import { countryDialingCodes, defaultCountryCode, priorityCountryCodes } from "../../constants";
...
export default function StartUpDetails({
  ...
  const form = useForm<z.infer<typeof startUpDetailsSchema>>({
    ...
    defaultValues: {
      ...
      // Always default to US if no existing value
      countryCode: startUpDetails.phone_number?.split(" ")[0] 
        ? Object.entries(countryDialingCodes).find(
            ([_, value]) => value === startUpDetails.phone_number?.split(" ")[0]
  ...
  // Helper function to check if a country code exists in priorityCountryCodes
  const isInPriorityCountries = (code: string): code is keyof typeof priorityCountryCodes => {
    return code in priorityCountryCodes;
  };
                    ...
                    <FormControl>
                      ...
                      >
                        <optgroup label="Common">
                          {(Object.keys(priorityCountryCodes) as Array<keyof typeof priorityCountryCodes>).map(
                            (code) => (
                              <option key={code} value={code}>
                                {code} {priorityCountryCodes[code]}
                              </option>
                            )
                          )}
                        </optgroup>
                        <optgroup label="All Countries">
                          {Object.entries(countryDialingCodes)
                            .filter(([code]) => !isInPriorityCountries(code))
                            .map(([code, dialCode]) => (
                              <option key={code} value={code}>
                                {code} {dialCode}
                              </option>
                  ...
                  <FormItem className="flex flex-col gap-1 flex-1">
                    <FormControl>
                      <input
                        ...
                        placeholder="(555) 555-5555"  // Updated placeholder to match format
                        {...field}
                        onChange={(e) => handlePhoneChange(e, field.onChange)}  // Add phone formatting
                        maxLength={14}  // Limit length to (XXX) XXX-XXXX format
                      />
                    </FormControl>
                    <FormMessage className="absolute text-red-600 -bottom-7" />
                  </FormItem>
